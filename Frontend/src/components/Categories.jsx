...
function Categories(){
    ...
    const updateCategory=cat=>{
        axios.put('http://localhost:8000/api/categories/'+cat.id,{isactive:!cat.isactive})
        .then(resp=>{
    ...
    const handleSubmit=e=>{
        ...
        axios.post('http://localhost:8000/api/categories',{catname:catname})
        .then(resp=>{
            setcatname('')
            enqueueSnackbar(resp.data.data, {
                variant: "success",
    ...
    const action = (snackbarId) => (
        <>
          <button onClick={() => {closeSnackbar(snackbarId);}}>OK</button>
        </>
      );
